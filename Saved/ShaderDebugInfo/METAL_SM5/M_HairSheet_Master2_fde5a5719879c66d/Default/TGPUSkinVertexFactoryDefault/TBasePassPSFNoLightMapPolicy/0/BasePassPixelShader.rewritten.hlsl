...
cbuffer Material {
	 ...
	 uint    BindlessSRV_Material_Texture2D_5 ;
	 uint    Material_Padding308 ;
	 uint    BindlessSampler_Material_Texture2D_5Sampler ;
	 uint    Material_Padding316 ;
	 uint    BindlessSRV_Material_Texture2D_6 ;
	 uint    Material_Padding324 ;
	 uint    BindlessSampler_Material_Texture2D_6Sampler ;
	 uint    Material_Padding332 ;
	 uint    BindlessSRV_Material_Texture2D_7 ;
	 uint    Material_Padding340 ;
	 uint    BindlessSampler_Material_Texture2D_7Sampler ;
	 uint    Material_Padding348 ;
	 uint    BindlessSampler_Material_Wrap_WorldGroupSettings ;
	 uint    Material_Padding356 ;
...
float WSToDF( float In) { return In; }
float2 DFToWS( float2 In) { return In; }
float2 WSToDF( float2 In) { return In; }
float3 DFToWS( float3 In) { return In; }
float3 WSToDF( float3 In) { return In; }
float4 DFToWS( float4 In) { return In; }
float4 WSToDF( float4 In) { return In; }
...
float2 CustomExpression0(FMaterialPixelParameters Parameters)
{
return View_TemporalAAParams;
}
float CustomExpression1(FMaterialPixelParameters Parameters,float2 p)
{
return ((uint)(p.x) + 2 * (uint)(p.y)) % 5;
}
...
float GetMaterialCustomData0(in out FMaterialPixelParameters Parameters)
{
	...
	float Local1 =  1.0f;
	float4 Local2 = ProcessMaterialLinearGreyscaleTextureLookup(Texture2DSampleBias(Material_Texture2D_0,GetMaterialSharedSampler(Material_Texture2D_0Sampler,View_MaterialTextureBilinearWrapedSampler),  Local0 ,Material_PreshaderBuffer[0].x));
	float Local3 =  1.0f;
	float3 Local4 = lerp(Material_PreshaderBuffer[0].yzw,Material_PreshaderBuffer[1].xyz,Local2.r);
	float3 Local5 = (Local4 + Material_PreshaderBuffer[2].xyz);
	float3 Local6 = normalize(Local5);
	float4 Local7 = MaterialCollection0.Vectors[16];
	float4 Local8 = lerp(float4(0.89999998,0.00000000,0.00000000,1.00000000).rgba,Local7.rgba,Material_PreshaderBuffer[2].w);
	float4 Local9 = (Local8 * ((float4)Material_PreshaderBuffer[3].x));
	float4 Local10 = (Local9 * ((float4)Material_PreshaderBuffer[3].y));
	float3 Local11 = (((float3)0.00000000) + Local10.xyz);
	float3 Local12 = max(Local11,((float3)0.00000000));
	float Local13 = GetPixelDepth(Parameters);
	float Local14 =   Local13 .r;
	float4 Local15 = MaterialCollection0.Vectors[1];
	float Local16 = (  Local14  - Local15.a);
	float Local17 = (  Local16  / 1200.00000000);
	float Local18 = saturate(  Local17 );
	float3 Local19 = (Parameters.CameraVector + float3(0.00000000,0.00000000,1.50000000).rgb);
	float3 Local20 = normalize(Local19);
	float Local21 = dot(Local20,Parameters.WorldNormal);
	float Local22 = saturate(Local21);
	float4 Local23 = MaterialCollection0.Vectors[0];
	float Local24 = (Local22 * Local23.g);
	float Local25 =  1.0f;
	float4 Local26 = ProcessMaterialLinearGreyscaleTextureLookup(Texture2DSample(Material_Texture2D_1,GetMaterialSharedSampler(Material_Texture2D_1Sampler,View_MaterialTextureBilinearWrapedSampler),  Local0 ));
	float Local27 =  1.0f;
	float3 Local28 = lerp(Material_PreshaderBuffer[4].xyz,Material_PreshaderBuffer[5].xyz,Local26.r);
	float Local29 =  1.0f;
	float4 Local30 = ProcessMaterialColorTextureLookup(Texture2DSample(Material_Texture2D_2,GetMaterialSharedSampler(Material_Texture2D_2Sampler,View_MaterialTextureBilinearWrapedSampler),  Local0 ));
	float Local31 =  1.0f;
	float3 Local32 = (Local28 * Local30.rgb);
	float2 Local33 = (  Local0  * ((float2)Material_PreshaderBuffer[5].w));
	float Local34 =  1.0f;
	float4 Local35 = ProcessMaterialLinearGreyscaleTextureLookup(Texture2DSample(Material_Texture2D_3,GetMaterialSharedSampler(Material_Texture2D_3Sampler,View_MaterialTextureBilinearWrapedSampler),  Local33 ));
	float Local36 =  1.0f;
	float3 Local37 = (Local32 * ((float3)Local35.r));
	float3 Local38 = (Local37 + Local32);
	float3 Local39 = (((float3)Material_PreshaderBuffer[6].x) * Local38);
	float3 Local40 = max(Local39,((float3)0.00100000));
	float3 Local41 = min(Local40,((float3)0.99900001));
	float3 Local42 = lerp(float3(0.80000001,0.00000000,0.00000000).rgb,((float3)1.00000000),Material_PreshaderBuffer[6].y);
	float3 Local43 = (Local41 * Local42);
	float Local44 = dot(Local43,float3(0.30000001,0.58999997,0.11000000));
	float3 Local45 = lerp(Local43,((float3)Local44),-2.00000000);
	float3 Local46 = float3(Parameters.TexCoords[1].x, Parameters.TexCoords[1].y, Parameters.TexCoords[2].x);
	float Local47 =   Local46 .b;
	float Local48 = (GetPrimitiveData(Parameters).ObjectRadius * 2.00000000);
	float Local49 = (  Local47  / Local48);
	float Local50 = (1.00000000 +   Local49 );
	float Local51 = (  Local50  * 0.50000000);
	float Local52 = lerp(-2.00000000,2.00000000,  Local51 );
	float Local53 = saturate(  Local52 );
	float3 Local54 = (Local12 + float3(0.00000000,0.00000000,0.00000000));
	float3 Local55 = max(Local54,((float3)0.00000000));
	float3 Local56 = lerp(((float3)0.00000000),Local55.rgb,Material_PreshaderBuffer[6].z);
	float3 Local57 = lerp(Local56.rgb.rgb,Material_PreshaderBuffer[7].xyz,Material_PreshaderBuffer[7].w);
	float3 Local58 = lerp(((float3)0.00000000),Local43.rgb,Material_PreshaderBuffer[6].z);
	float3 Local59 = max(Local58.rgb.rgb,((float3)0.00100000));
	float3 Local60 = min(Local59,((float3)1.00000000));
	float3 Local61 = lerp(((float3)0.00000000),Material_PreshaderBuffer[8].x.rrr,Material_PreshaderBuffer[6].z);
	float3 Local62 = lerp(((float3)0.00000000),Material_PreshaderBuffer[8].y.rrr,Material_PreshaderBuffer[6].z);
	float3 Local63 = lerp(((float3)0.80000001),Material_PreshaderBuffer[8].z.rrr,Material_PreshaderBuffer[6].z);
	float Local64 =  1.0f;
	float4 Local65 = ProcessMaterialLinearGreyscaleTextureLookup(Texture2DSample(Material_Texture2D_4,GetMaterialSharedSampler(Material_Texture2D_4Sampler,View_MaterialTextureBilinearWrapedSampler),  Local0 ));
	float Local66 =  1.0f;
	float Local67 = PositiveClampedPow(Local65.r,2.00000000);
	float Local68 = dot(Parameters.CameraVector,Parameters.TangentToWorld[2]);
	float Local69 = abs(Local68);
	float Local70 = (Local69 * 1.50000000);
	float Local71 = lerp(Material_PreshaderBuffer[8].w,Material_PreshaderBuffer[9].x,Local70);
	float Local72 = saturate(Local71);
	float Local73 = lerp(Material_PreshaderBuffer[9].y,1.00000000,Local72.r);
	float Local74 = lerp(Local67,1.00000000,Local73);
	float Local75 =  1.0f;
	float4 Local76 = ProcessMaterialLinearGreyscaleTextureLookup(Texture2DSampleBias(Material_Texture2D_5,GetMaterialSharedSampler(Material_Texture2D_5Sampler,View_MaterialTextureBilinearWrapedSampler),  Local0 ,Material_PreshaderBuffer[0].x));
	float Local77 =  1.0f;
	float Local78 = (Local74 * Local76.r);
	float Local105 = PositiveClampedPow(Local26.r,Material_PreshaderBuffer[11].w);
	float Local106 = lerp(Material_PreshaderBuffer[12].x,Material_PreshaderBuffer[12].y,Local105);
	float Local107 = lerp(0.00000000,Material_PreshaderBuffer[12].z,Local65.r);
	float Local108 = dot(Parameters.TangentToWorld[2],Parameters.CameraVector);
	float Local109 = abs(Local108);
	float Local110 = saturate(Local109);
	float Local111 = (1.00000000 - Local110);
	float Local112 = PositiveClampedPow(Local111,Material_PreshaderBuffer[12].w);
	float Local113 = lerp(Local107,1.00000000,Local112);
	float Local114 = lerp(1.00000000,Local113,Material_PreshaderBuffer[13].x);
	float2 Local131 = GetPixelPosition(Parameters);
	float2 Local132 = CustomExpression0(Parameters);
	float2 Local133 = (Local131 + ((float2)Local132.r));
	float Local134 = CustomExpression1(Parameters,Local133);
	float2 Local135 = (Local131 / float2(64.00000000,64.00000000).rg);
	float Local136 = (fmod(View_GameTime,1.00000000) * 0.10000000);
	float2 Local137 = (Local135 + ((float2)Local136));
	float Local138 =  1.0f;
	float4 Local139 = ProcessMaterialLinearGreyscaleTextureLookup(Texture2DSampleBias(Material_Texture2D_3,Material_Texture2D_3Sampler,Local137,View_MaterialTextureMipBias));
	float Local140 =  1.0f;
	float Local141 = (Local134 + Local139.r);
	float Local142 = (Local141 / 6.00000000);
	float Local143 = (1.00000000 - Local65.r);
	float Local144 = (Local142 + Local143);
	float Local145 = (Local144 + -0.50000000);
	float Local146 = (Local145 * Material_PreshaderBuffer[13].y);
	return Local106.rrr.rgb.rgb;;
}
...
void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
	...
	float3 Local5 = (Local4 + Material_PreshaderBuffer[2].xyz);
	float3 Local6 = normalize(Local5);
	PixelMaterialInputs.Normal = Local6.rgb.rgb.rgb;
	...
	PixelMaterialInputs.OpacityMask = Local78.rrr.rgb.rgb;
	PixelMaterialInputs.BaseColor = Local60;
	PixelMaterialInputs.Metallic = Local61.rgb.rgb;
	PixelMaterialInputs.Specular = Local62.rgb.rgb;
	PixelMaterialInputs.Roughness = Local63.rgb.rgb;
	PixelMaterialInputs.Anisotropy = 0.00000000;
	PixelMaterialInputs.Normal = Local6.rgb.rgb.rgb;
	PixelMaterialInputs.Tangent = float3(1.00000000,0.00000000,0.00000000);
	PixelMaterialInputs.Subsurface = 0;
	PixelMaterialInputs.AmbientOcclusion = Local114.rrr.rgb.rgb;
	PixelMaterialInputs.Refraction = 0;
	PixelMaterialInputs.PixelDepthOffset = Local146.rrr.rgb.rgb;
...
float ApplyPixelDepthOffsetToMaterialParameters(inout FMaterialPixelParameters MaterialParameters, FPixelMaterialInputs PixelMaterialInputs, out float OutDepth)
{
	...
	MaterialParameters.LWCData.AbsoluteWorldPosition = LWCAdd(MaterialParameters.LWCData.AbsoluteWorldPosition, -MaterialParameters.CameraVector * PixelDepthOffset);
	MaterialParameters.AbsoluteWorldPosition = WSToDF(MaterialParameters.LWCData.AbsoluteWorldPosition);
	OutDepth =  MakePrecise( DeviceDepth );
	...
	const uint ShadingModel)
{
	...
	else if (ShadingModel == 7)
	{
		...
		GBuffer.CustomData.z = saturate( GetMaterialCustomData0(MaterialParameters) );	
	}
}
