...
cbuffer Material {
	 float4    Material_PreshaderBuffer[7] ;
...
void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
	...
	float3 Local20 = lerp(float3(0.80000001,0.00000000,0.00000000).rgb,((float3)1.00000000),Material_PreshaderBuffer[1].w);
	float3 Local21 = (Material_PreshaderBuffer[1].xyz * Local20);
	...
	float3 Local25 = float3(Parameters.TexCoords[1].x, Parameters.TexCoords[1].y, Parameters.TexCoords[2].x);
	...
	float3 Local37 = lerp(((float3)0.00000000),Local36.rgb,Material_PreshaderBuffer[2].x);
	float3 Local38 = lerp(Local37.rgb.rgb,Material_PreshaderBuffer[2].yzw,Material_PreshaderBuffer[3].x);
	float3 Local39 = lerp(((float3)0.00000000),Local21.rgb,Material_PreshaderBuffer[2].x);
	float2 Local40 = Parameters.TexCoords[0].xy;
	float Local41 =  1.0f;
	float4 Local42 = ProcessMaterialColorTextureLookup(Texture2DSampleBias(Material_Texture2D_0,Material_Texture2D_0Sampler,  Local40 ,View_MaterialTextureMipBias));
	float Local43 =  1.0f;
	float3 Local44 = PositiveClampedPow(Local42.rgb,((float3)Material_PreshaderBuffer[3].y));
	float3 Local45 = (Local44 * ((float3)Material_PreshaderBuffer[3].z));
	float Local46 = CalcSceneDepth(ScreenAlignedPosition(GetScreenPosition(Parameters)));
	float Local47 = (Local46 -   Local6 );
	float Local48 = (Local47 * Material_PreshaderBuffer[3].w);
	float Local49 = saturate(Local48);
	float3 Local50 = (Local45 * ((float3)Local49));
	float3 Local51 = (Local50 * ((float3)Material_PreshaderBuffer[1].w));
	float3 Local52 = (((float3)Material_PreshaderBuffer[2].x) * Local51.rgb);
	PixelMaterialInputs.EmissiveColor = Local38;
	PixelMaterialInputs.Opacity = Local52.rgb.rgb;
	...
	PixelMaterialInputs.Refraction = float3(float3(1.00000000,0.00000000,0.00000000).xy,Material_PreshaderBuffer[6].w);
