...
{	
	...
	if (_statCom->IsDead() && player != nullptr)
	{
		SetActorEnableCollision(false);
		auto controller = GetController();
		if (controller)
			GetController()->UnPossess();
		player->GetStatCom()->AddExp(GetExp());
		player->GetInvenCom()->AddMoney(_Dropmoney);

		if (player->GetInvenWidget())
		{
			player->GetInvenWidget()->MoneyUpdate(player->GetInvenCom()->GetMoney());
		}
	}
